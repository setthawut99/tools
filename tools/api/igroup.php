...
foreach ($innodb as $token){
	...
	$me = json_decode($bclass->curl("https://graph.facebook.com/me?access_token=".urlencode($token)), TRUE);

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/".$_POST["group"]."/members?access_token=".urlencode($token)."&method=POST&member=".$me["id"]);
...
exit(json_encode(array("status" => "success" ,"msg" => 'ขอเข้ากลุ่มสำเร็จ')));