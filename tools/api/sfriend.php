...
foreach ($innodb as $token){
	    $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/me/friends?access_token=".urlencode($token));
		...
		foreach ($output["data"] as $friend){
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://graph.facebook.com/".$friend."/feed?link=https://www.facebook.com/".$_POST['idpost']."&message=".urlencode("#".$bclass->random(rand(10,20)))."&method=post&access_token=".urlencode($token));
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			$status = curl_exec($ch);
			curl_close($ch); 
